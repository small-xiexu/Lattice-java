...
CREATE TABLE IF NOT EXISTS articles (
    ...
    requires_result_verification BOOLEAN NOT NULL DEFAULT FALSE,
...
COMMENT ON COLUMN articles.risk_level IS '内容风险等级，独立于审查状态';
COMMENT ON COLUMN articles.risk_reasons IS '内容风险原因数组';
COMMENT ON COLUMN articles.is_hotspot IS '是否属于高频热点内容';
COMMENT ON COLUMN articles.requires_result_verification IS '是否需要结果级抽检';
...
CREATE INDEX IF NOT EXISTS idx_articles_refkey_text_hash
    ON articles ((md5(refkey_text)));

CREATE INDEX IF NOT EXISTS idx_articles_risk_level
    ON articles (risk_level);

CREATE INDEX IF NOT EXISTS idx_articles_risk_reasons
    ON articles USING GIN (risk_reasons);

CREATE INDEX IF NOT EXISTS idx_articles_hotspot_verification
    ON articles (is_hotspot, requires_result_verification);

CREATE TABLE IF NOT EXISTS article_usage_stats (
    ...
    answer_feedback_count INTEGER NOT NULL DEFAULT 0,
...
COMMENT ON TABLE article_usage_stats IS '文章命中、引用和反馈热度统计表';
COMMENT ON COLUMN article_usage_stats.article_key IS '文章唯一键';
COMMENT ON COLUMN article_usage_stats.concept_id IS '概念标识';
COMMENT ON COLUMN article_usage_stats.retrieval_hit_count IS '进入融合结果的检索命中次数';
COMMENT ON COLUMN article_usage_stats.citation_count IS '最终答案中引用该文章的次数';
COMMENT ON COLUMN article_usage_stats.answer_feedback_count IS '答案反馈关联该文章或其来源路径的次数';
COMMENT ON COLUMN article_usage_stats.manual_mark_count IS '既有人工或外部热点标记次数';
COMMENT ON COLUMN article_usage_stats.heat_score IS '通用热度分';
COMMENT ON COLUMN article_usage_stats.source_paths IS '文章来源路径数组';
COMMENT ON COLUMN article_usage_stats.updated_at IS '最近一次统计刷新时间';

CREATE INDEX IF NOT EXISTS idx_article_usage_stats_heat_score
    ON article_usage_stats (heat_score DESC, updated_at DESC);
...
CREATE TABLE IF NOT EXISTS pending_answer_feedback (
    id BIGSERIAL PRIMARY KEY,
    query_id VARCHAR(64),
    ...
    article_keys TEXT[] NOT NULL DEFAULT ARRAY[]::TEXT[],
    source_paths TEXT[] NOT NULL DEFAULT ARRAY[]::TEXT[],
    reported_by VARCHAR(128) NOT NULL DEFAULT 'anonymous',
...
COMMENT ON TABLE pending_answer_feedback IS '答案结果反馈队列表';
COMMENT ON COLUMN pending_answer_feedback.id IS '答案反馈主键 ID';
COMMENT ON COLUMN pending_answer_feedback.query_id IS '关联查询 ID';
COMMENT ON COLUMN pending_answer_feedback.question IS '用户问题';
COMMENT ON COLUMN pending_answer_feedback.answer_summary IS '答案摘要或原始回答截断';
COMMENT ON COLUMN pending_answer_feedback.feedback_type IS '反馈类型';
COMMENT ON COLUMN pending_answer_feedback.comment IS '用户反馈说明';
COMMENT ON COLUMN pending_answer_feedback.article_keys IS '反馈关联的文章唯一键数组';
COMMENT ON COLUMN pending_answer_feedback.source_paths IS '反馈关联的来源路径数组';
COMMENT ON COLUMN pending_answer_feedback.reported_by IS '反馈提交人';
COMMENT ON COLUMN pending_answer_feedback.status IS '反馈处理状态';
COMMENT ON COLUMN pending_answer_feedback.resolution_comment IS '处理结论说明';
COMMENT ON COLUMN pending_answer_feedback.handled_by IS '处理人';
COMMENT ON COLUMN pending_answer_feedback.handled_at IS '处理时间';
COMMENT ON COLUMN pending_answer_feedback.created_at IS '反馈创建时间';
COMMENT ON COLUMN pending_answer_feedback.updated_at IS '反馈更新时间';
COMMENT ON COLUMN pending_answer_feedback.metadata_json IS '反馈扩展元数据 JSON';

CREATE INDEX IF NOT EXISTS idx_pending_answer_feedback_status_created_at
    ON pending_answer_feedback (status, created_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS idx_pending_answer_feedback_query_id
    ON pending_answer_feedback (query_id);

CREATE INDEX IF NOT EXISTS idx_pending_answer_feedback_article_keys
    ON pending_answer_feedback USING GIN (article_keys);

CREATE INDEX IF NOT EXISTS idx_pending_answer_feedback_source_paths
    ON pending_answer_feedback USING GIN (source_paths);

CREATE TABLE IF NOT EXISTS answer_feedback_audits (
    id BIGSERIAL PRIMARY KEY,
    feedback_id BIGINT NOT NULL REFERENCES pending_answer_feedback (id) ON DELETE CASCADE,
...
COMMENT ON TABLE answer_feedback_audits IS '答案反馈处理审计表';
COMMENT ON COLUMN answer_feedback_audits.id IS '答案反馈审计主键 ID';
COMMENT ON COLUMN answer_feedback_audits.feedback_id IS '关联答案反馈 ID';
COMMENT ON COLUMN answer_feedback_audits.action IS '处理动作';
COMMENT ON COLUMN answer_feedback_audits.previous_status IS '处理前状态';
COMMENT ON COLUMN answer_feedback_audits.next_status IS '处理后状态';
COMMENT ON COLUMN answer_feedback_audits.comment IS '处理说明';
COMMENT ON COLUMN answer_feedback_audits.operated_by IS '操作人';
COMMENT ON COLUMN answer_feedback_audits.operated_at IS '操作时间';
COMMENT ON COLUMN answer_feedback_audits.metadata_json IS '处理扩展元数据 JSON';

CREATE INDEX IF NOT EXISTS idx_answer_feedback_audits_feedback_operated_at
    ON answer_feedback_audits (feedback_id, operated_at DESC, id DESC);
...
CREATE TABLE IF NOT EXISTS structured_tables (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    source_path_norm TEXT NOT NULL DEFAULT '',
    search_tsv TSVECTOR NOT NULL DEFAULT ''::tsvector
);

COMMENT ON TABLE structured_tables IS '通用结构化表格主表';
COMMENT ON COLUMN structured_tables.id IS '结构化表格主键 ID';
COMMENT ON COLUMN structured_tables.source_file_id IS '关联源文件主键 ID';
COMMENT ON COLUMN structured_tables.source_id IS '所属资料源主键 ID';
COMMENT ON COLUMN structured_tables.source_path IS '源文件相对路径';
COMMENT ON COLUMN structured_tables.table_name IS '表格名称';
COMMENT ON COLUMN structured_tables.sheet_name IS 'Excel sheet 名称或同等表名';
COMMENT ON COLUMN structured_tables.format IS '结构化表格来源格式';
COMMENT ON COLUMN structured_tables.header_row_number IS '表头原始行号';
COMMENT ON COLUMN structured_tables.row_count IS '数据行数';
COMMENT ON COLUMN structured_tables.column_count IS '列数';
COMMENT ON COLUMN structured_tables.metadata_json IS '表格扩展元数据 JSON';
COMMENT ON COLUMN structured_tables.source_path_norm IS '源路径归一化检索文本';
COMMENT ON COLUMN structured_tables.search_tsv IS '表格全文检索 tsvector';

CREATE INDEX IF NOT EXISTS idx_structured_tables_source_file_id
    ON structured_tables (source_file_id);

CREATE INDEX IF NOT EXISTS idx_structured_tables_source_id
    ON structured_tables (source_id);

CREATE INDEX IF NOT EXISTS idx_structured_tables_source_path
    ON structured_tables (source_path);

CREATE INDEX IF NOT EXISTS idx_structured_tables_source_path_norm
    ON structured_tables (source_path_norm);

CREATE INDEX IF NOT EXISTS idx_structured_tables_search_tsv
    ON structured_tables USING GIN (search_tsv);

CREATE TABLE IF NOT EXISTS structured_table_rows (
    id BIGSERIAL PRIMARY KEY,
...
COMMENT ON TABLE structured_table_rows IS '通用结构化表格行表';
COMMENT ON COLUMN structured_table_rows.id IS '结构化表格行主键 ID';
COMMENT ON COLUMN structured_table_rows.table_id IS '关联结构化表格主键 ID';
COMMENT ON COLUMN structured_table_rows.row_number IS '源表格原始行号';
COMMENT ON COLUMN structured_table_rows.row_text IS '带列名的行文本';
COMMENT ON COLUMN structured_table_rows.metadata_json IS '行扩展元数据 JSON';
COMMENT ON COLUMN structured_table_rows.search_tsv IS '行全文检索 tsvector';

CREATE UNIQUE INDEX IF NOT EXISTS uk_structured_table_rows_table_row
    ON structured_table_rows (table_id, row_number);

CREATE INDEX IF NOT EXISTS idx_structured_table_rows_search_tsv
    ON structured_table_rows USING GIN (search_tsv);

CREATE TABLE IF NOT EXISTS structured_table_cells (
    id BIGSERIAL PRIMARY KEY,
    table_id BIGINT NOT NULL REFERENCES structured_tables (id) ON DELETE CASCADE,
    row_id BIGINT NOT NULL REFERENCES structured_table_rows (id) ON DELETE CASCADE,
    row_number INTEGER NOT NULL,
    ...
    metadata_json JSONB NOT NULL DEFAULT '{}'::JSONB,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    column_name_norm TEXT NOT NULL DEFAULT '',
    search_tsv TSVECTOR NOT NULL DEFAULT ''::tsvector
);

COMMENT ON TABLE structured_table_cells IS '通用结构化表格单元格表';
COMMENT ON COLUMN structured_table_cells.id IS '结构化表格单元格主键 ID';
COMMENT ON COLUMN structured_table_cells.table_id IS '关联结构化表格主键 ID';
COMMENT ON COLUMN structured_table_cells.row_id IS '关联结构化行主键 ID';
COMMENT ON COLUMN structured_table_cells.row_number IS '源表格原始行号';
COMMENT ON COLUMN structured_table_cells.column_index IS '源表格原始列序号';
COMMENT ON COLUMN structured_table_cells.column_name IS '列名';
COMMENT ON COLUMN structured_table_cells.cell_value IS '单元格原始值';
COMMENT ON COLUMN structured_table_cells.normalized_value IS '单元格归一化值';
COMMENT ON COLUMN structured_table_cells.value_type IS '单元格值类型';
COMMENT ON COLUMN structured_table_cells.metadata_json IS '单元格扩展元数据 JSON';
COMMENT ON COLUMN structured_table_cells.column_name_norm IS '列名归一化检索文本';
COMMENT ON COLUMN structured_table_cells.search_tsv IS '单元格全文检索 tsvector';

CREATE UNIQUE INDEX IF NOT EXISTS uk_structured_table_cells_row_column
    ON structured_table_cells (row_id, column_index);

CREATE INDEX IF NOT EXISTS idx_structured_table_cells_table_row
    ON structured_table_cells (table_id, row_number);

CREATE INDEX IF NOT EXISTS idx_structured_table_cells_column_value
    ON structured_table_cells (column_name_norm, normalized_value);

CREATE INDEX IF NOT EXISTS idx_structured_table_cells_value_type
    ON structured_table_cells (value_type);

CREATE INDEX IF NOT EXISTS idx_structured_table_cells_search_tsv
    ON structured_table_cells USING GIN (search_tsv);
...
COMMENT ON TABLE fact_cards IS '结构化事实证据卡表';
COMMENT ON COLUMN fact_cards.id IS '事实证据卡主键 ID';
COMMENT ON COLUMN fact_cards.card_id IS '事实证据卡稳定业务标识';
COMMENT ON COLUMN fact_cards.source_id IS '所属资料源主键 ID';
COMMENT ON COLUMN fact_cards.source_file_id IS '所属源文件主键 ID';
COMMENT ON COLUMN fact_cards.card_type IS '事实证据卡类型';
COMMENT ON COLUMN fact_cards.answer_shape IS '面向查询的答案形态';
COMMENT ON COLUMN fact_cards.title IS '事实证据卡标题';
COMMENT ON COLUMN fact_cards.claim IS '事实证据卡直接支撑的核心结论';
COMMENT ON COLUMN fact_cards.items_json IS '事实证据卡结构化条目 JSON';
COMMENT ON COLUMN fact_cards.evidence_text IS '可回指原文的证据文本';
COMMENT ON COLUMN fact_cards.source_chunk_ids IS '回指的源文件分块主键数组';
COMMENT ON COLUMN fact_cards.article_ids IS '关联背景文章主键数组';
COMMENT ON COLUMN fact_cards.confidence IS '证据卡置信度';
COMMENT ON COLUMN fact_cards.review_status IS '证据卡审查状态';
COMMENT ON COLUMN fact_cards.content_hash IS '证据卡内容哈希';
COMMENT ON COLUMN fact_cards.created_at IS '创建时间';
COMMENT ON COLUMN fact_cards.updated_at IS '更新时间';
COMMENT ON COLUMN fact_cards.search_tsv IS '事实证据卡全文检索 tsvector';

CREATE UNIQUE INDEX IF NOT EXISTS uk_fact_cards_card_id
    ON fact_cards (card_id);

CREATE INDEX IF NOT EXISTS idx_fact_cards_source_file_id
    ON fact_cards (source_file_id);

CREATE INDEX IF NOT EXISTS idx_fact_cards_card_type
    ON fact_cards (card_type);

CREATE INDEX IF NOT EXISTS idx_fact_cards_answer_shape
    ON fact_cards (answer_shape);

CREATE INDEX IF NOT EXISTS idx_fact_cards_review_status
    ON fact_cards (review_status);

CREATE INDEX IF NOT EXISTS idx_fact_cards_content_hash
    ON fact_cards (content_hash);
...
CREATE TABLE IF NOT EXISTS article_review_audits (
    id BIGSERIAL PRIMARY KEY,
    source_id BIGINT,
    article_key VARCHAR(256),
    concept_id VARCHAR(255) NOT NULL,
    action VARCHAR(64) NOT NULL,
    previous_review_status VARCHAR(64),
...
COMMENT ON TABLE article_review_audits IS '文章人工复核审计表';
COMMENT ON COLUMN article_review_audits.id IS '审计主键 ID';
COMMENT ON COLUMN article_review_audits.source_id IS '所属资料源主键 ID';
COMMENT ON COLUMN article_review_audits.article_key IS 'source-aware 文章唯一键';
COMMENT ON COLUMN article_review_audits.concept_id IS '文章概念唯一标识';
COMMENT ON COLUMN article_review_audits.action IS '人工复核动作';
COMMENT ON COLUMN article_review_audits.previous_review_status IS '复核前审查状态';
COMMENT ON COLUMN article_review_audits.next_review_status IS '复核后审查状态';
COMMENT ON COLUMN article_review_audits.comment IS '复核意见';
COMMENT ON COLUMN article_review_audits.reviewed_by IS '复核人';
COMMENT ON COLUMN article_review_audits.reviewed_at IS '复核时间';
COMMENT ON COLUMN article_review_audits.metadata_json IS '复核扩展元数据 JSON';

CREATE INDEX IF NOT EXISTS idx_article_review_audits_article_key_reviewed_at
    ON article_review_audits (article_key, reviewed_at DESC, id DESC);
...
CREATE TABLE IF NOT EXISTS query_retrieval_settings (
    ...
    fact_card_weight NUMERIC NOT NULL DEFAULT 1.40,
...
COMMENT ON COLUMN query_retrieval_settings.fact_card_weight IS 'Fact Card lexical 召回权重';
...
CREATE TABLE IF NOT EXISTS query_retrieval_runs (
    ...
    answer_shape VARCHAR(64) NOT NULL DEFAULT 'GENERAL',
    ...
    fact_card_hit_count INTEGER NOT NULL DEFAULT 0,
    source_chunk_hit_count INTEGER NOT NULL DEFAULT 0,
    coverage_status VARCHAR(32) NOT NULL DEFAULT 'missing',
    channel_run_summary_json JSONB NOT NULL DEFAULT '{}'::jsonb,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
COMMENT ON COLUMN query_retrieval_runs.answer_shape IS '答案形态';
...
COMMENT ON COLUMN query_retrieval_runs.fact_card_hit_count IS 'Fact Card 通道命中数';
COMMENT ON COLUMN query_retrieval_runs.source_chunk_hit_count IS 'Source Chunk 通道命中数';
COMMENT ON COLUMN query_retrieval_runs.coverage_status IS '结构化证据覆盖状态';
COMMENT ON COLUMN query_retrieval_runs.channel_run_summary_json IS '通道运行状态摘要 JSON';
...
COMMENT ON COLUMN query_retrieval_channel_hits.fact_card_id IS 'Fact Card 数据库主键';
COMMENT ON COLUMN query_retrieval_channel_hits.card_type IS 'Fact Card 类型';
COMMENT ON COLUMN query_retrieval_channel_hits.review_status IS '证据审查状态';
COMMENT ON COLUMN query_retrieval_channel_hits.confidence IS 'Fact Card 置信度';
COMMENT ON COLUMN query_retrieval_channel_hits.source_chunk_ids_json IS 'Fact Card 回指 source chunk id JSON 数组';
...
BEGIN
    ...
    IF vector_type_name IS NOT NULL THEN
        ...
        EXECUTE format(
            'CREATE TABLE IF NOT EXISTS fact_card_vector_index (
                fact_card_id BIGINT PRIMARY KEY REFERENCES fact_cards (id) ON DELETE CASCADE,
        ...
        CREATE INDEX IF NOT EXISTS idx_fact_card_vector_index_card_type
            ON fact_card_vector_index (card_type, answer_shape);

        CREATE INDEX IF NOT EXISTS idx_fact_card_vector_index_updated_at
            ON fact_card_vector_index (updated_at DESC);
...
BEGIN
    ...
    END IF;
    IF to_regclass(current_schema() || '.fact_card_vector_index') IS NOT NULL THEN
        EXECUTE 'COMMENT ON TABLE fact_card_vector_index IS ''事实证据卡向量索引表''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.fact_card_id IS ''关联事实证据卡主键 ID''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.card_id IS ''关联事实证据卡稳定业务标识''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.card_type IS ''事实证据卡类型''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.answer_shape IS ''答案形态''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.model_profile_id IS ''Embedding profile 主键''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.embedding_dimensions IS ''向量维度''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.index_version IS ''索引版本''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.content_hash IS ''证据卡内容哈希''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.embedding IS ''事实证据卡向量嵌入''';
        EXECUTE 'COMMENT ON COLUMN fact_card_vector_index.updated_at IS ''向量索引更新时间''';
    END IF;
END $$;
