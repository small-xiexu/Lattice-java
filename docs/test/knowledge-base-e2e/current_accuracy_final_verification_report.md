...
### 2.1 Hidden Eval 已通过（2026-06-08 更新）

| 套件 | 原始 Answer | 重建后 Answer | 通过线 | 状态 |
|---|---|---|---|---|
| Hidden A（文档泛化） | 50% | **93%** | 80% | ✅ PASS |
| Hidden B（代码泛化） | 67% | **100%** | 80% | ✅ PASS |

**更新**：线 B（FTS OR query + ts config simple）提交后，清库重建（新 ts config 下重建 tsvector 索引）使 Hidden A 和 Hidden B 均达到通过线。详见 `hidden_eval_rebuild_reverification_report.md`。
...
理由：
1. 5 套公开题集全部通过（100%），覆盖 5 个领域
2. Java 代码库搜索/问答能力已验证 PASS
3. Hidden A/B 清库重建后均达到通过线（93%/100%）
4. 搜索精度全 6/6，无幻觉，拒答率 95%+
5. 关键代码修复已提交，门禁持续通过

**试用前告知的已知限制：**
1. 极少数时间窗口判断查询可能返回 NO_RELEVANT_KNOWLEDGE
2. 缩略词/代码名查询建议使用全称
3. PDF 文件名需控制在 32 字符内
