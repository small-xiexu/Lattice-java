...
- 2026-05-29 进行中：继续 Phase 1E-1 alias enricher interface 骨架验证；当前只验证已存在未提交改动，不扩大到真实 LLM alias、prompt、query fallback 或 citation。
- 2026-05-29 已完成，待 agentD 独立验证：Phase 1E-2 真实 LLM Field Alias Enricher runtime、prompt 加载与最小单测已落地；`git diff --check`、redline、定向测试与全量 `mvn test` 已通过；本轮未清库、未重建 schema、未导入资料、未跑业务 eval，下一步交由 agentD 做独立 runtime / 端到端验证。
- 2026-05-29 代码修复已完成，待处理全量测试独立阻塞：Phase 1E-2 runtime wiring 阻塞已按最小范围修复，`LlmFactCardTerminalUnitFieldAliasEnricher` 拆为 public Spring bean，`field-alias-enricher` 已加入 compile role 白名单；未改变 alias 策略、prompt、过滤逻辑或 fail-closed 行为。`git diff --check`、redline、alias/prompt 定向测试、API/UI role 定向测试通过；全量 `mvn test` 跑完 985 个测试，因 `ChatClientRegistryTests.shouldCacheAndIsolateDynamicChatClientsAcrossRoutes` 报 `NonTransientAiException: 404` 出现 1 个 error，本轮按边界未扩大修改。未清库、未重建 schema、未导入资料、未跑业务 eval。
- 2026-05-30 已完成：Phase 1E-2 runtime wiring gate 复跑 PASS；`git diff --check`、redline、alias/prompt 定向测试、API/UI role 定向测试、`ChatClientRegistryTests` 独立复跑与全量 `mvn test` 均通过，全量结果 `985/0/0/0`，上一轮 404 判定为偶发波动。详见 `terminal_unit_phase1e_runtime_wiring_gate_rerun_report.md`。
- 2026-05-30 阻塞：Phase 1E-2 runtime smoke rerun BLOCKED；`compile + field-alias-enricher` DB binding 已存在且 API 白名单已通过，但 `LlmFactCardTerminalUnitFieldAliasEnricher` 调用无 scope `routeResolution("compile", "field-alias-enricher")` 时只命中 bootstrap fallback，未命中有效 binding / snapshot，导致 `isLlmRouteAvailable()` 返回 false，真实 LLM alias 未触发。下一步按 `terminal_unit_phase1e_llm_route_resolution_analysis_report.md` 交 agentA 做 scoped route 最小修复。
- 2026-05-30 已完成，待 agentD 复跑 runtime smoke：Phase 1E-2 scoped alias route 最小修复已落地；compile jobId 从 `PersistSourceFileChunksNode` → `SourceIngestSupport` → `FactCardGenerationService` → `LlmFactCardTerminalUnitFieldAliasEnricher` 的传递链路已打通；有 scope 时使用 `routeResolutionFor` + `generateTextWithScope`，无 scope 时保持 fail-closed。`git diff --check`、redline BLOCKER=0、定向测试 36/0/0 通过。全量 `mvn test=987/0/0/0` 通过。本轮未清库、未重建 schema、未导入资料、未跑业务 eval。详见 `terminal_unit_phase1e_scoped_alias_route_fix_result_report.md`。
- 2026-05-30 已完成（agentD clean schema e2e）：Phase 1E-2 clean schema 端到端验证 PARTIAL。YAML 5/5 目标 terminal unit 首次全部进入 fused topK（基线 0/5），排名大幅改善（FQ3: 9→2, FQ6: >5→2, FG2: 4→2）。LLM alias 全部正确生成（5/5 目标字段）。但最终答案 0/5 PASS——FALLBACK evidence selector 未消费 terminal unit evidence，选中了整卡 ARTICLE 证据。根因已从 retrieval 层下移到 answer consumption 层。详见 `terminal_unit_phase1e_clean_schema_e2e_verification_report.md`。
- 2026-05-30 进行中（agentA）：Phase 1E terminal content / description 增强。根因：terminal unit content 不含 fieldAliases，extractDescription 不识别 fieldDescription。修复：FactCardTerminalUnitMapper.xml content 扩展 + QueryEvidenceRelevanceSupport fieldDescription 识别。本轮不改 fallback selector gate。
- 2026-05-30 已完成，待 agentD clean schema / runtime 复验：Phase 1E terminal content / description 增强已落地。`FactCardTerminalUnitMapper.xml` content 从 `display_text + field_description` 扩展为含 `field_aliases_json::text`；`QueryEvidenceRelevanceSupport.extractDescription()` 新增 `fieldDescription` 键名 fallback。`git diff --check`、redline BLOCKER=0、全量 `mvn test=987/0/0/0` 通过。preferArticleEvidence 丢弃 FACT_CARD 的潜在后续风险已明确记录。本轮未清库、未重建 schema、未导入资料、未跑业务 eval。详见 `terminal_unit_phase1e_terminal_content_enhancement_fix_result_report.md`。
- 2026-05-30 进行中（agentA）：Phase 1E terminal evidence consumption 修复。根因：preferArticleEvidence=true 路径无条件丢弃 fact_card_terminal_fts 产生的 FACT_CARD terminal unit。修复：AnswerFallbackEvidenceSelector 通用消费高置信 terminal unit evidence。FQ6/FG2 borrowing_system 目标 unit fused rank=1 但最终答案仍选错片段。
- 2026-05-30 已完成，待 agentD clean schema / runtime 复验：Phase 1E terminal evidence consumption 修复已落地。`AnswerFallbackEvidenceSelector` 在 structured fact / exact lookup 问题中允许 `fact_card_terminal_fts` channel 的 terminal unit FACT_CARD 通过 `preferArticleEvidence=true` 过滤。`git diff --check`、redline BLOCKER=0、定向测试 6/0/0、全量 `mvn test=987/0/0/0` 通过。snippet selection 未修改（留作独立变量）。本轮未清库、未重建、未导入、未跑业务 eval。详见 `terminal_unit_phase1e_terminal_evidence_consumption_fix_result_report.md`。
- 2026-05-30 进行中（agentA）：Phase 1E terminal evidence consumption 测试补强。为 AnswerFallbackEvidenceSelector 新增 terminal unit 豁免分支补充 synthetic 单测。不修改生产代码。
- 2026-05-30 已完成，待 agentD clean schema / runtime 复验：Phase 1E terminal conclusion consumption 修复已落地。`AnswerFallbackConclusionBuilder` 在 structured fact / exact lookup 问题中优先从 terminal unit 提取 `displayText` exact line 作为结论。`git diff --check`、redline BLOCKER=0、定向测试 18/0/0 (11 selector + 7 conclusion)、全量 `mvn test=997/0/0/0` 通过。本轮未修改 selector、snippet selection、citation。详见 `terminal_unit_phase1e_terminal_conclusion_consumption_fix_result_report.md`。
- 2026-05-30 已完成（agentB 只读归因）：Phase 1E answer consumption 分析。agentB 已完成 AnswerFallbackEvidenceSelector / QueryEvidenceRelevanceSupport 只读归因，定位三个通用缺口：(1) terminal unit content 字段（display_text + field_description）不含 fieldAliases，导致 fallback 评分极低；(2) preferArticleEvidence=true 路径按 evidenceType 丢弃所有 FACT_CARD 命中；(3) extractDescription() 简易字符串匹配不识别 fieldDescription 键名。推荐 agentA 做最小 content 增强修复（仅改 FactCardTerminalUnitMapper.xml + QueryEvidenceRelevanceSupport.extractDescription()，不改 fallback gate）。详见 `terminal_unit_phase1e_answer_consumption_analysis_report.md`。
- 2026-05-30 已完成（agentD content / description 增强验证）：Phase 1E terminal content / description 增强验证 PASS；YAML 5 题从 0/5 提升到 3/5，Answer Accuracy 10/15 提升到 13/15。FQ6 / FG2 仍 FAIL，目标 terminal unit 已进入 fused topK，但最终答案未消费 borrowing_system 的 exact terminal values。详见 `terminal_unit_phase1e_terminal_content_enhancement_verification_report.md`。
- 2026-05-30 已完成（agentB 只读归因）：FQ6 / FG2 borrowing_system 剩余失败归因完成。当前唯一推荐下一步为 fallback / answer consumption 对 terminal unit FACT_CARD evidence 的通用消费规则，而不是优先继续加强编译期 article summary 覆盖；交 agentA 做 answer consumption 单变量最小修复，完成后交 agentD 做 clean schema / runtime 验证。详见 `terminal_unit_phase1e_borrowing_system_failure_analysis_report.md`。
