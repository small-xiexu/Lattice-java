...
更新时间：2026-05-17（persist gate 测试补强后更新）
...
- 下阶段：agentD 做 persist gate pre-commit 质量复核。

## 当前 Gate

| 项 | 当前状态 | 说明 |
|---|---|---|
| redline | `BLOCKER=0` | persist gate 测试补强后：`BLOCKER=0 / REVIEW=1351 / ALLOWLIST=166`。最终以 pre-commit 复核 REV/ALLOWLIST 结果为准。 |
| mvn test | `812/0/0` 通过 | persist gate 测试补强后：新增 `PersistArticlesNodeTests`（1 个 case），全量 812 通过。测试库已隔离到 `ai-rag-knowledge-test`。 |
...
| agentC | 项目进度台账与文档治理 | 已完成 persist gate 台账更新与报告输出 | 否，除文档/报告 |
| agentD | 验证/测试 | 负责 redline、`mvn test`、baseline、业务 eval 验证报告；已完成 compile review observability + persist gate 验证；下一步做 persist gate pre-commit 质量复核 | 否，除验证报告 |
...
- Query visibility hard filter 仍是下一阶段事项，不能和本轮 persist gate 混修。当前未开启 LLM reviewer。
...
9. （当前）agentD 做 persist gate pre-commit 质量复核：确认 redline BLOCKER=0、mvn test=812/0/0、工作区只含允许变更。
10. （后续）单独处理 Query visibility hard filter，不和本轮 persist gate 混修。
11. （后续）如要启用 LLM reviewer，必须单独设计并验证 persist/query 可见性门禁，不能假设当前 rule-based 的可见性对 LLM reviewer 同样有效。
