...
- [x] Phase F：全面超越证据补齐
  ...
  - 进行中：按“修到可对外更强口径”为目标收口真实短板；已重新部署包含 `AnswerGenerationService` 配置题修复与 `IncrementalCompileService` 传播收紧修复的最新构建物。`2026-04-29` 续跑时发现 `18090` 启动脚本仍指向隔离 schema `lattice_e2e_codex_20260427`，但该 schema 已被外部清理，导致上一轮 full compile 现场无法直接续接。当前下一步改为先重建这套隔离 schema 并重新做一轮 full compile 建基线，再执行单文件受控增量复验，重点确认 `payments` 不再被 `ops/postmortem.md` 跨文章污染、`referentialKeywords` 不再混入 `51` / `21 分钟` / `GW-504-RETRY-EXHAUSTED`。
