...
- [ ] D4 补齐向量配置防呆与重建引导
  ...
  - 进行中：已补“向量配置变更 / 向量索引重建后自动清空 query cache”，避免向量修复后继续命中旧缓存答案。
  - 验收：管理员在 UI 上即可看懂“是否兼容、为什么不兼容、下一步该做什么”。

- [ ] D5 提升架构原因类中文问答的召回稳定性
  ...
  - 进行中：已确认“有召回但回答当前证据不足/暂无法确认”的负向结果仍可能被缓存；本轮已补“负向问答结果不缓存 + compile 成功后自动失效 query cache”。
  ...
  - 下一步：扩大中文“为什么 / 原因”类样本面，判断该问题是否已收敛为缓存与向量失效导致的个例，而非仍需单独调权重。
  - 验收：对 `ADR-012-order-to-inventory-via-mq.md` 相关中文问法能稳定命中并给出有引用的回答。

- [ ] D6 优化 `/admin/ask` 的答案与证据联动体验
...
- [ ] D7 优化 `/admin/developer-access` 与 `/admin` 首页信息架构
...
- [ ] D11 启动 `ChatClient + Advisor` Phase 0 Spike
  ...
  - 进行中：已完成文档方案、三轮架构复审与实施边界收敛，待开始 D11-1 路径核对。
  - 下一步：先做 D11-1，确认 `LlmGateway`、`ChatModelLlmClient`、`LlmClientFactory` 三者当前分工，再落最小动态构造 Spike。
...
- 当前阶段：D1-D3 已完成，D4 进行中，D5 进行中，D10 已完成，D11 待启动
- 已完成：本轮真实回归、问题分级、改造范围与执行顺序已沉淀为清单；向量 schema 自动对齐、重建链路与缓存失效已补齐；原先失败的中文 ADR 问法已在真实实例恢复正确回答；负向问答结果不再写入 query cache，compile 完成后也会自动清空旧 query cache；`ChatClient + Advisor` 详细技术方案文档已落地，并已补充 `ChatClient` 归属澄清、《全链路日志与追踪设计》以及基于 Claude 三轮评审后的双层缓存、phase、`traceId`、日志验收、后备路线、Phase 0/0.5 说明修订与 `non-LLM` 路径 outcome 映射；Claude 三次复审提示词已同步准备；Phase 0 Spike 已拆解为 D11 可执行清单
- 下一步：从 D11-1 开始，先核对当前 bootstrap / snapshot 调用路径与 Spike 真正边界，再进入最小动态 `OpenAiChatModel + ChatClient` 构造验证
